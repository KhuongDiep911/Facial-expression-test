...
OUTPUT_FOLDER_NAME = "fer2013_dataset"
    ...
    for i in range(len(samples)):
        try:
            if labels[i] in SELECTED_LABELS and nb_images_per_label[get_new_label(labels[i])] < IMAGES_PER_LABEL:
                ...
                print(category)
                print(i)
                imgplot = plt.imshow((image*255).astype(np.uint8))
                plt.show()
                if SAVE_IMAGES:
                    # imageio.imwrite(category + '/' + str(i) + '.jpg', image)
                    # scipy.misc.imsave(category + '/' + str(i) + '.jpg', image)
                    imsave(category + '/' + str(i) + '.jpg', (image*255).astype(np.uint8))
                    print(category + '/' + str(i) + '.jpg')
                ...
                if GET_LANDMARKS:
                    ...
                    imsave('temp.jpg', (image*255).astype(np.uint8))
