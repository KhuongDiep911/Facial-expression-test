...
def train(optimizer=HYPERPARAMS.optimizer, optimizer_param=HYPERPARAMS.optimizer_param, 
    ...
    if train_model:
        training, validation = load_data(validation=True)
    else:
        training, validation, test = load_data(validation=True, test=True)

    with tf.Graph().as_default():
        print("building model...")
        network = build_model(optimizer, optimizer_param, learning_rate, learning_rate
        ...
        if train_model:
            ...
            if NETWORK.use_landmarks:
                model.fit([training['X'], training['X2']], training['Y'], 
